# Step 2 — DDT Syntax Conversion

Every use case from `output/step-1/step-1.md` converted into a DoubleDots attribute value, grouped under the same 16 category headers.

## Sources and conventions

- Triggers are grounded in: `src/1b_DomEvents.js` (DOM event array), `src/1c_WindowDocumentEvents.js` (`dcl`, window/document-only events), `src/1d_navigationViewport.js` (`viewport-resize`, `viewport-scroll`, `orientation`, `navigate`), `src/AttributeObserver/PortalObservers.js` (`i`, `attr_<name>`, `resize`, `content-box`, `border-box`, `device-pixel-content-box`, `intersection`, `intersectionPrevious` — which `PortalMap.define()` kebab-cases to `intersection-previous`), and `src/4_Portals.js` + `src/AttributeObserver/PortalState.js` (`state_<prop>`, `prevent`, `log`, `er`).
- `.dot` KebabReflection (doc/11_The dot-rules.md): native DOM methods and properties such as `:.focus`, `:.remove`, `:.play`, `:.close`, `:.show-modal`, `:.request-submit`, `:.report-validity`, `:.toggle-attribute_open`, `:.set-attribute_disabled_true`, `:.remove-attribute_hidden`, getters like `:.value` and `:.id`, and setters like `:.text-content_copied` and `:.muted_true`. Per doc/11 point 4, KebabReflection also invokes custom methods on web components; chains below use this for app-level effects (`:.load-more`, `:.recalculate`, filter-style methods like `:.is-other` paired with the `:.` truthy-pass filter). Arguments `e` and `oi` are substituted with the event and the previous output.
- `-dash` targeting (doc/12_The dash-rules.md): `-p` / `-p--form` / `-p--body` (up), `-c0` (child), `-n` / `-m` (siblings), `-t` (from event target), `--.panel` (descendant), `--data-x` (attribute query on current element), `:-` (currentElement), `:-.` (event), `:-..` (oi). Dash rules are static: each one re-originates from the element carrying the attribute.
- Leading `_` marks a global/document-level trigger (doc/8_custom_triggers.md, doc/c_syntax.md soft rule 1): used wherever the listener must catch the event regardless of which element it hit (`_keydown_escape`, `_click`, `_selectionchange`).
- `_` inside a name passes an argument (doc/c_syntax.md soft rule 2): `keydown_ctrl_k`, `visibilitychange_hidden`, `orientation_portrait`, `pageshow_persisted`, `intersection_off` (the `off` option documented in `src/4_Portals.js`), `scroll_passive` / `wheel` (`_passive` variant documented in `src/1b_DomEvents.js`).
- State store (src/4_Portals.js, src/AttributeObserver/PortalState.js, test26/7_state.html): `state_<prop>` or `state_<prop1>_<prop2>` triggers fire when a stored prop changes; the dispatched input is the store's State object, read with `:-..:.<prop>`; a chain ending in the `state_<prop>` reaction writes to the store.
...
1. A long press on a conversation row in a mobile messaging app enters multi-select mode (category 1) — no long-press gesture trigger exists in source; gestures are documented only as a custom StateMachineAttr pattern in doc/18_gestures.md, with no registered long-press portal.
2. A quick tap on a smart home light tile toggles the light, while a long press expands the tile (category 1) — same missing long-press gesture trigger; the tap half alone would misrepresent the use case.
3. A user clicks a copy invite link button in a team admin page and the label briefly reads copied as confirmation feedback (category 1) — writing to the clipboard (navigator.clipboard) is not reachable: `.dot` KebabReflection only invokes methods on elements/attributes, and no clipboard-write reaction is registered.
4. A user copies a code block from documentation using its copy button (category 5) — same clipboard-write gap; the `copy` event trigger only fires for user-initiated copies, not for programmatic writes from a button click.
5. When a badge's text content changes to a new unread count, the badge briefly pulses (category 8) — the `attr` portal is attributes-only (`{attributes: true}` in src/AttributeObserver/PortalObservers.js); no portal exposes characterData mutations.
6. When a third-party chat widget injects its launcher button into the page, the site's own help link hides (category 8) — no portal exposes childList/DOM-structure mutations.
7. When a screen-reader live region gains new text, the region announces it (category 8) — same characterData gap.
8. When the content of a stock ticker cell changes, the cell flashes green or red (category 8) — same characterData gap.
9. When an ad blocker strips the creative from an ad container, the container collapses itself (category 8) — same childList gap.
10. When a sorting library reorders table rows in the DOM, the row numbers rewrite themselves (category 8) — same childList gap.
11. When a browser extension injects a node into a reading app, the app's layout observer nudges content (category 8) — same childList gap.
12. When a spinner's animation is cancelled because a request failed, an error card replaces it (category 11) — `animationcancel` is not in the DOM event array in src/1b_DomEvents.js (only `transitioncancel` is).
13. When a kiosk page has been idle and visible for hours, a scheduled overnight reload refreshes its content (category 13) — no timer/scheduler trigger exists; setTimeout/setInterval are deprecated per doc/a_API.md and the `wait` portal is commented out in src/dd.js.
14. When document fonts finish loading in a typography-heavy magazine layout, headlines re-measure (category 13) — no FontFaceSet/document-fonts portal exists in source.
15. As a shopper types a username during signup, an availability check runs after each short pause (category 3) — no debounce/throttle reaction exists in source, and approximating with a plain `input` trigger would drop the defining pause behavior.
16. A nurse typing in a patient search box sees results filter only after typing pauses briefly (category 3, seen in: Alpine.js) — same debounce gap.
17. A live clock label ticks once per second for exactly as long as its dashboard widget stays on screen (category 15, seen in: Svelte) — the per-second tick needs an interval trigger, which does not exist in source (timers deprecated, no wait portal registered).

## Count verification

- step-1.md total use cases: 271
- converted above: 254
- unmapped above: 17
- 254 + 17 = 271 (all use cases accounted for)
